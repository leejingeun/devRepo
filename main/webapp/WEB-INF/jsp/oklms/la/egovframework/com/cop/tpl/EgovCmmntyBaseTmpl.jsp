...
<!--
	function fn_egov_goCmmntyHome(){
		var url = "<c:url value='/la/cop/cmy/CmmntyMainContents.do'/>";
	...
	function fn_egov_loadBdList(bbsId, bbsAttrbCode, bbsTyCode){
		...
		if(bbsTyCode == 'BBST04'){ // 방명록의 경우
			url = "<c:url value='/la/cop/bbs/selectGuestList.do' />?bbsId="+bbsId+"&bbsAttrbCode="+bbsAttrbCode;
		}else if(bbsTyCode == 'BBST02'){ // 익명게시판의 경우 (2011.9.7 수정분)
			url = "<c:url value='/la/cop/bbs/anonymous/selectBoardList.do' />?bbsId="+bbsId+"&bbsAttrbCode="+bbsAttrbCode;
		}else{
			url = "<c:url value='/la/cop/bbs/selectBoardList.do' />?bbsId="+bbsId+"&bbsAttrbCode="+bbsAttrbCode;
		}
		document.getElementById("contentFrame").src = url;
	}
	...
	function fn_egov_loadClub(clbId){
		...
		document.frm.action = "<c:url value='/la/cop/cus/ClubMainPage.do'/>";
		document.frm.submit();
	}

	function fn_egov_registUser(cmmntyId){
		window.open("<c:url value='/la/cop/cmy/insertCmmntyUserBySelf.do' />?cmmntyId="+cmmntyId, "userRegist", "width=320px, height=200px;");
	}

	function fn_egov_deleteUser(cmmntyId){
		//window.open("<c:url value='/la/cop/cmy/deleteCmmntyUserBySelf.do' />?cmmntyId="+cmmntyId, "userDeletet", "width=320px, height=200px;");
		window.open("<c:url value='/la/cop/com/selectCmmntyMngrList.do' />?trgetId="+cmmntyId+"&PopFlag=S", "userDelete", "width=640px, height=400px;");
	}
//-->
</script>
</c:otherwise>
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		<c:when test="${preview == 'true'}">
        	<!--  컨텐츠 영역 : Start -->
			<iframe id="contentFrame" onload="javascript:calcHeight();" src="<c:url value='/la/cop/cmy/previewCmmntyMainContents.do'/>" frameborder="0" width="758" height="480" title="컨텐츠영역"></iframe>
        	<!--  컨텐츠 영역 : End -->
        </c:when>
        <c:otherwise>
        	<!--  컨텐츠 영역 : Start -->
			<iframe id="contentFrame" onload="javascript:calcHeight();" src="<c:url value='/la/cop/cmy/CmmntyMainContents.do'/>?cmmntyId=<c:out value='${cmmntyVO.cmmntyId}' />" frameborder="0" width="758" height="480" title="컨텐츠영역"></iframe>
        	<!--  컨텐츠 영역 : End -->
        </c:otherwise>
        </c:choose>
          <br />
