...
<script type="text/javascript">
	...
	function fn_egov_select_confirmList(pageNo) {
		...
		if (target == '') {
			actionUrl = "<c:url value='/la/cop/com/selectConfirmRequest.do'/>";
		} else {
			actionUrl = "<c:url value='/la/cop/com/selectConfirmRequestByTrget.do'/>";
		}
	...
	function fn_egov_inqire_confirmInfo(cnfmNo,trgetId) {
		...
		document.frm.action = "<c:url value='/la/cop/com/forUpdateConfirmRequest.do'/>";
		document.frm.submit();
	}
</script>
<title>승인 목록</title>
...
<form name="frm" action ="<c:url value='/la/cop/com/forUpdateConfirmRequest.do' />" method="post">
		    ...
		    <td class="lt_text3" nowrap>
		    	<c:choose>
		    		<c:when test="${result.confmSttusCode == 'AP01'}">
						<input type="hidden" name="confmNumber" value="0" />
						<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
						<!-- a href="<c:url value='/la/cop/cmy/selectCmmntyInf.do'/>?confmNumber=0"; onclick="">
							<c:out value="${result.confmTyCodeNm}"/>
						</a-->
						<!-- a href="<c:url value='/la/cop/com/forUpdateConfirmRequest.do'/>?confmNumber=0"; onclick="">
							<c:out value="${result.confmTyCodeNm}"/>
						</a-->
						<!-- 2011.9.7 수정분 -->
