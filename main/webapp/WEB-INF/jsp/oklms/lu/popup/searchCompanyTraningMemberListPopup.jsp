...
<script type="text/javascript">
	...
	function fn_selectInfo(){
		if( opener ){
			...
			if(undefined == obj){
				alert("사용자정보를 셋팅할 하나의 기업체를 선택하여주십시오. ")
				return false;
			}

			opener.fn_setMemberInfo(obj);
...
<div id="pop-wrapper" class="min-w650">
	...
	<div class="search-box-1 mb-010">
		<input type="text" id="searchKeyword" name="searchKeyword" style="width:200px" placeholder="성명을 입력" />
	...
	<table class="type-2">
		...
		<thead>
			...
			<c:forEach var="result" items="${resultSeachList}" varStatus="status">
			<tr>
				<td><input type="radio" name="tempId" id="tempId" value="${result.memSeq}||${result.memId}||${result.memName}"></td>
				<td>${result.memId}</td>
			...
			<c:if test="${fn:length(resultSeachList) == 0}">
			<tr>
