						...
						<dl id="tab-type">
							...
							<dd id="tab-content-11" style="display:block;">
								...
								<div class="search-box-1 mt-020">
									기업:<input type="text" name="searchCompanyName" id="searchCompanyName" value="${activityVO.searchCompanyName }"/>
									성명:<input type="text" name="searchName" id="searchName" value="${activityVO.searchName}"/>
									<a href="#!" onclick="javascript:fn_comp_search_search();">검색</a>
								</div><!-- E : search-box-1 -->

								<ul id="learner-wrap" class="mt-030 mb-020">
									<li id="prev" onclick="myScroll.scrollToPage('prev', 0);return false">prev</li>
									<li id="wrap">
										<!-- 기업수 * 128px의 값을 아래 style width에 넣어줘야함 -->
										<div id="scroller" style="width:${fn:length(companylist)*128}px;">
											<ul id="thelist" class="name-tab-btns">

											<c:forEach var="result" items="${companylist}" varStatus="status">
												<c:if test="${result.classId eq activityVO.classId }">
													<li class="current"  ><a href="#!"  id="company_0" onclick="javascript:fn_comp_search('${result.classId }');">${result.classId }반_${result.companyName }</a></li>
												</c:if><c:if test="${result.classId ne activityVO.classId}">
													<li><a href="#!" onclick="javascript:fn_comp_search('${result.classId }');">${result.classId }반_${result.companyName }</a></li>
												</c:if>
											</c:forEach>												
								...
								<div class="group-area name-tab-contents">
									<h4 class="mb-010"><span>${currProcVO.subClass }반_${currProcVO.subjectName }  </span> ㅣ ${currProcVO.yyyy}학년도 / ${currProcVO.term}학기</h4>
...
<input type="hidden" name="searchName" id="searchMemName" value=""/>
									...
									<tr>
										...
										<td class="left">${activityVO.classId }반_${result.companyName } </td>
							...
							<dd id="tab-content-12"> 
								...
								<ul id="learner-wrap" class="mt-030 mb-010">
										...
										<div id="scroller" style="width:${fn:length(memberlist)*128}px;">
											...
											<c:forEach var="result" items="${memberlist}" varStatus="status">
												<c:if test="${status.index eq '0' }">
													<li id="current_${status.count}" class="current"><a href="#!" id="ntest">${result.memName }</a></li>
									...
									<li id="next" onclick="myScroll.scrollToPage('next', 0);">next</li>
...
<div id="">									
									<table class="type-1-1 mt-010">
										...
										<tbody>
											<tr>
												<td>
													<select   name="weekCnt"   onchange="javascript:fn_Search(this.value);">
								...
								<tr>
									...
									<table class="type-write mt-005" style="border-top:1px solid #DDD;">
										...
										<tbody>
											...
											<c:if test="${!empty memberVO.atchFileId and !memberVO.atchFileId eq '' }">
