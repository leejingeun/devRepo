...
<input type="hidden" name="classId" value="${currProcVO.subClass}" />
						...
						<table class="type-write">
							...
							<tbody>
									...
									<td class="left">
										<select id="weekCnt" name="weekCnt"  >
											<c:forEach var="result" items="${onlineTraningSchVO}" varStatus="status">
												<option value="${result.weekCnt}" <c:if test="${reportVO.weekCnt eq result.weekCnt}"> selected </c:if>>${result.weekCnt}</option>
											</c:forEach>
										</select>&nbsp;&nbsp;&nbsp;
										주차
									</td>
								...
								<tr>
									<th>제목</th>
									<td class="left"><input type="text" name="reportName" style="width:97%;" value="${reportVO.reportName }"/></td>
								</tr>
								<tr>
									<th>과제제출 기간</th>
									<td class="left">
										...
										<input type="text" name="submitStartDate"  value="${reportVO.submitStartDate}" style="width:70px" />  ~ <input type="text" name="submitEndDate"  value="${reportVO.submitEndDate}" style="width:70px" />
									</td>
								</tr>
								<tr>
									<th>평가</th>
									<td class="left">
										<input type="radio" name="evalYn" value="Y" class="choice" <c:if test="${reportVO.evalYn eq 'Y'}">checked</c:if> />&nbsp;&nbsp;Y&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="radio" name="evalYn" value="N" class="choice" <c:if test="${reportVO.evalYn eq 'N'}">checked</c:if>/>&nbsp;&nbsp;N
									</td>
								</tr>
								<tr>
									<th>배점</th>
									<td class="left"><input type="number" name="score" id="score" value="${reportVO.score }"  min="0" max="9" style="width:30px; text-align:right;"  /> 점
									</td>
								</tr>
								<tr>
									<th>내용</th>
						...
						<div class="btn-area align-right mt-010">
							<a href="/lu/report/listReport.do" class="gray-1 float-left">취소</a><a href="#" onclick="javascript:fn_formSave();" class="orange">저장</a>
						</div><!-- E : btn-area -->
