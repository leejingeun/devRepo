...
<ul id="learner-wrap" class="mt-030 mb-010">
	...
	<li id="wrap">
		...
		<div id="scroller" style="width:${fn:length(resultSubjectClassList)*128}px;">
			...
			<td class="left">
				...
				<c:choose>
						<c:when test="${result.isExpired == 'T' && result.isExpired !='Y'}">
							<c:if test="${'Y' == result.topNoticeYn}">
								<span class="course">[<c:out value="${result.subjectName}"/>]</span> <B><c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</B>
							</c:if>
							<c:if test="${'N' == result.topNoticeYn}">
								<span class="course">[<c:out value="${result.subjectName}"/>]</span> <c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]
							</c:if>
						</c:when>
						<c:when test="${result.isExpired !='T' && result.isExpired =='Y'}">
							<c:if test="${'Y' == result.topNoticeYn}">
